...
public class phishing_sa {
    ...
    private static int inputLayer = 30, hiddenLayer=2, outputLayer = 1;
    ...
    public static void main(String[] args) {
        ...
        double[] coolings = {0.01};

        double[] T = {1E5, 1E11, 1E15};

        for (int trainingIterations : iterations) {
            results = "";
            for (double t : T) {
                double start = System.nanoTime(), end, trainingTime, testingTime, correct = 0, incorrect = 0;
                oa[0] = new SimulatedAnnealing(t, 0.01, nnop[0]);
                ...
                results += "\nTrain Results for SA:" + t + ": \nCorrectly classified " + correct + " instances." +
                        "\nIncorrectly classified " + incorrect + " instances.\nPercent correctly classified: "
                        + df.format(correct / (correct + incorrect) * 100) + "%\nTraining time: " + df.format(trainingTime)
                        + " seconds\nTesting time: " + df.format(testingTime) + " seconds\n";

                final_result = oaNames[0] + "," + trainingIterations + "," + t + "," + "training accuracy" + "," + df.format(correct / (correct + incorrect) * 100)
                ...
                results += "\nTest Results for SA: " + t + ": \nCorrectly classified " + correct + " instances." +
                        "\nIncorrectly classified " + incorrect + " instances.\nPercent correctly classified: "
                        + df.format(correct / (correct + incorrect) * 100) + "%\nTraining time: " + df.format(trainingTime)
                        + " seconds\nTesting time: " + df.format(testingTime) + " seconds\n";

                final_result = oaNames[0] + "," + trainingIterations + "," + t + "," + "testing accuracy" + "," + df.format(correct / (correct + incorrect) * 100)
                        + "," + "training time" + "," + df.format(trainingTime) + "," + "testing time" +
                        "," + df.format(testingTime);
                write_output_to_file("Optimization_Results", "phishing_results_sa.csv", final_result, true);
            }
